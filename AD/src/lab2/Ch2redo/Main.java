...
import lab2.Ch2redo.Actor;
import lab2.Ch2redo.Film;
import lab2.Ch2redo.Premiu;
import lab2.Ch2redo.Studio;

import java.util.ArrayList;
import java.util.List;

public class Main {
    public static String[] getStudioWithNumberOfMoviesCriteria(Studio[] database, Integer moviesNumber){
        String[] studios = new String[10];
        int i=0;
        for(Studio currentStudio:database){
            if(currentStudio.count() >= moviesNumber){
                if (i==studios.length){
                    String[] newStudios = new String[studios.length+5];
                    for(int j=0;j<i;j++){
                        newStudios[j] = studios[j];
                    }
                    studios = newStudios;
                }
                studios[i++] = currentStudio.getNume();
            }
        }
        return studios;
    }
    ...
    public static void main(String[] args) {
        ...
        for (Studio studio:studioDatabase){
            if (studio != null){
                String[] filmName = studio.getFilmNameWithActorAboveAge(50);
                if (filmName != null){
                    System.out.println(String.join(", ",filmName));
